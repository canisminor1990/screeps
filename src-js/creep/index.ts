...
mod.extend = function() {
	...
	Creep.prototype.run = function(behaviour) {
		if (!this.spawning) {
			...
			if (global.DEBUG && global.TRACE)
				trace('Creep', { creepName: this.name, pos: this.pos, Behaviour: behaviour && behaviour.name, Creep: 'run' });
			if (behaviour) {
				behaviour.run(this);
				p.checkCPU('behaviour.run', PROFILING.MIN_THRESHOLD);
			} else if (!this.data) {
				if (global.DEBUG && global.TRACE)
					trace('Creep', { creepName: this.name, pos: this.pos, Creep: 'run' }, 'memory init');
	...
	Creep.prototype.fleeMove = function() {
		if (global.DEBUG && global.TRACE)
			trace('Creep', { creepName: this.name, pos: this.pos, Action: 'fleeMove', Creep: 'run' });
		let drop = r => {
			...
			if (
				...
				if (idleFlag) {
					...
					let goals = this.room.structures.all
						...
						.concat(
							this.room.sources.map(function(s) {
								return { pos: s.pos, range: 2 };
							}),
						)
						.concat(
							this.pos.findInRange(FIND_EXIT, 2).map(function(e) {
								return { pos: e, range: 1 };
							}),
						)
						.concat(
							this.room.myConstructionSites.map(function(o) {
								return { pos: o.pos, range: 1 };
							}),
						);
		...
		if (this.carry.energy > 0 && this.hasActiveBodyparts(WORK)) {
			const repairRange =
				this.data && this.data.creepType === 'remoteHauler'
					? global.REMOTE_HAULER.DRIVE_BY_REPAIR_RANGE
					: global.DRIVE_BY_REPAIR_RANGE;
			const repairTarget = _(this.pos.findInRange(FIND_STRUCTURES, repairRange)).find(s =>
	...
	Creep.prototype.buildNearby = function() {
		...
		const buildTarget = _(
			this.pos.findInRange(FIND_MY_CONSTRUCTION_SITES, global.REMOTE_HAULER.DRIVE_BY_BUILD_RANGE),
		).find(
			s =>
				global.REMOTE_HAULER.DRIVE_BY_BUILD_ALL ||
				(s.structureType === STRUCTURE_CONTAINER || s.structureType === STRUCTURE_ROAD),
		);
		if (buildTarget) {
			if (global.DEBUG && global.TRACE)
				trace('Creep', { creepName: this.name, Action: 'building', Creep: 'buildNearby' }, buildTarget.pos);
			this.build(buildTarget);
		} else {
			if (global.DEBUG && global.TRACE)
				trace('Creep', { creepName: this.name, Action: 'building', Creep: 'buildNearby' }, 'not building');
		}
	};
