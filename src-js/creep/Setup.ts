let Setup = function(typeName) {
	...
	this.SelfOrCall = function(obj, param) {
		...
		if (typeof obj === 'function') return obj.apply(this, [param]);
		else return obj;
	};
	...
	this.maxCost = function(room) {
		let c = this;
		return (
			Creep.bodyCosts(c.SelfOrCall(this._multiBody, room)) * c.SelfOrCall(this._maxMulti, room) +
			Creep.bodyCosts(c.SelfOrCall(this._fixedBody, room))
		);
	};
};
