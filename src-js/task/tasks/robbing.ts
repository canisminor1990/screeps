...
mod.checkFlag = flag => {
	...
	return (
		!(flag.room && flag.room.my) && flag.compareTo(FLAG_COLOR.invade.robbing) && Task.nextCreepCheck(flag, mod.name)
	);
};
// for each flag
...
mod.checkForRequiredCreeps = flag => {
	...
	if (count < (memory.numRobbers || 2)) {
		const spawnRoom = mod.state.robber.spawnRoom({ roomName });
		if (!spawnRoom) {
			return;
		}
		// robbers set homeRoom if closer storage exists
		const storageRoom = ROBBER_REHOME ? mod.state.robber.homeRoom(flag) : spawnRoom;
		Task.spawn(
