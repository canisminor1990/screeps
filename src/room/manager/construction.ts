	...
	prototypeExtend = () => {
		this.assignRoomPrototype({
			...
			roadConstructionTrace: {
				...
				set(value: any) {
					this.memory.roadConstructionTrace = value;
				},
			},
			getBestConstructionSiteFor: {
				value(pos, filter = null) {
					let sites;
			...
			roadConstruction: {
				value(minDeviation = ROAD_CONSTRUCTION_MIN_DEVIATION) {
					const forced =
					...
					data = data.filter(e => {
						if (e.n >= min) {
							...
							return (
								(structures.length === 0 || structures[0].structureType === STRUCTURE_RAMPART) &&
			...
			processConstructionFlags: {
				value() {
					...
					const CONSTRUCT = (flag, type) => {
						...
						const structures = POS.lookFor(LOOK_STRUCTURES).filter(
							s => !(s instanceof StructureRoad || s instanceof StructureRampart),
						);
					...
					let shortAmount =
						...
						(this.structures.extensions.length +
							_.filter(this.constructionSites, s => s.structureType === STRUCTURE_EXTENSION)
								.length);
					if (shortAmount > 0) {
					...
					// Spawns
					shortAmount =
						CONTROLLER_STRUCTURES[STRUCTURE_SPAWN][LEVEL] -
					...
					// Links
					shortAmount =
						CONTROLLER_STRUCTURES[STRUCTURE_LINK][LEVEL] -
					...
					// Labs
					shortAmount =
						CONTROLLER_STRUCTURES[STRUCTURE_LAB][LEVEL] -
					...
					// Power Spawn
					if (
						!this.structures.powerSpawn &&
						CONTROLLER_STRUCTURES[STRUCTURE_POWER_SPAWN][LEVEL] > 0
					) {
