	...
	prototypeExtend = () => {
		this.assignRoomPrototype({
					...
					for (let i = 0; i < orders.length; i++) {
						...
						if (amountRemaining <= 0) {
							...
							for (let j = 0; j < rooms.length; j++) {
								...
								// Util.logSystem(room.name, `${room.name} ${available} ${order.type}`);
								if (available < MIN_OFFER_AMOUNT) continue;

								// for COMPOUNDS_TO_ALLOCATE
								if (!_.isUndefined(COMPOUNDS_TO_ALLOCATE[order.type])) {
									let reservedAmount =
										COMPOUNDS_TO_ALLOCATE[order.type].amount +
										COMPOUNDS_TO_ALLOCATE[order.type].threshold;
									if (available < reservedAmount + MIN_OFFER_AMOUNT) continue;
									else available = available - reservedAmount;
								}

								if (amountRemaining < MIN_OFFER_AMOUNT && amountRemaining > 0)
									amountRemaining = MIN_OFFER_AMOUNT;
			...
			fillARoomOrder: {
				value() {
					if (
						!(this.terminal && this.memory && this.memory.resources && this.memory.resources.offers)
					)
						return false;
					...
					for (let i = 0; i < offers.length; i++) {
						...
						if (targetOfferIdx == -1) {
							Util.logSystem(this.name, 'Orphaned offer found and deleted');
							offers.splice(i--, 1);
							continue;
						}
					...
					if (containerData) {
						...
						if (existingOrder) {
							...
							if (container.structureType === STRUCTURE_LAB) {
								containerStore =
									container.mineralType == resourceType ? container.mineralAmount : 0;
							} else {
							...
							if (
								container.structureType === STRUCTURE_LAB &&
								containerData.reactionState != 'Storage'
							) {
			...
			terminalBroker: {
				value() {
					...
					for (const mineral in this.terminal.store) {
						...
						if ((MAKE_COMPOUNDS || ALLOCATE_COMPOUNDS) && mineral !== this.memory.mineralType)
							continue;
						...
						if (this.terminal.store[mineral] >= MIN_MINERAL_SELL_AMOUNT) {
							...
							let orders = Game.market.getAllOrders(o => {
								...
								o.ratio =
									(o.credits - o.transactionCost * ENERGY_VALUE_CREDITS) / o.transactionAmount; // best offer assuming 1e == ENERGY_VALUE_CREDITS credits
								...
								if (DEBUG || SELL_NOTIFICATION)
									Util.logSystem(
										...
										`Selling ${order.transactionAmount} ${mineral} for ${Util.roundUp(
											...
											order.transactionCost
										} e): ${Util.translateErrorCode(result)}`,
									);
