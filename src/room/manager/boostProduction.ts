				...
				if (data.orders.length === 0 || _.sum(data.orders, 'amount') === 0) {
					if (boostTiming.roomState === 'ordersPlaced') {
						...
						if (reactionMaking) {
							...
							Util.logSystem(room.name, `${room.name} orders done.`);
						}
					}
				}
					...
					if (labResourcesA === reactionAmount && labResourcesB === reactionAmount) {
						...
						if (reactionAmount > 0) {
							if (
								...
								Util.logSystem(room.name, `resourcesA: ${resourcesA} resourcesB: ${resourcesB}`);
								Util.logSystem(room.name, `reactionOrders fixed: ${reactionOrder.amount}`);
							}
						}
					} else if (reactionAmount > labResourcesA || reactionAmount > labResourcesB) {
						Util.logSystem(room.name, `NOT ENOUGH lab orders:`);
						Util.logSystem(
							room.name,
						...
						Util.logSystem(room.name, `TOO MUCH lab orders:`);
						Util.logSystem(
							room.name,
							`${room.name} reactionAmount: ${reactionAmount} DIFF: labA: ${labResourcesA -
								reactionAmount} labB: ${labResourcesB - reactionAmount}`,
