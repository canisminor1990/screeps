...
export function getRoadPathBetween(
  pos1: RoomPosition,
  pos2: RoomPosition,
...
function findRoadPath(
  ...
  cMatrix: (roomName: string) => CostMatrix | boolean
  ...
  let allowedRooms: string[] = [origin.pos.roomName].concat(
    RoomRepository.getAllOutposts(Game.rooms[origin.pos.roomName])
  );
  ...
  return PathFinder.search(
    origin.pos,
    { pos: destination.pos, range: options.range },
    {
...
export function getRoomCallbackForRoadbuilding(
  roomName: string,
  allowSK: boolean = false
): CostMatrix {
  let room = Game.rooms[roomName];
  ...
  if (allowSK === true) {
    room
      ...
      .forEach(function(lair: Structure) {
        let x: number, y: number, t: string;
        for (x = -2; x < 3; x++) {
          for (y = -2; y < 3; y++) {
