...
class WithdrawingAction extends CreepAction {
	constructor() {
		super('withdrawing');
		this.setDefault({
			isValidAction: creep => {
	...
	newTarget = creep => {
		...
		const targets = [];
		if (terminal && this.isValidTarget(terminal)) {
			targets.push(terminal);
		} else if (storage && this.isValidTarget(storage)) {
	...
	assignDebounce = (creep, outflowActions, target) => {
		const withdrawTarget = target || this.newTarget(creep);
		if (withdrawTarget) {
			if (
				withdrawTarget instanceof StructureStorage &&
				creep.data.lastAction === 'storing' &&
				creep.data.lastTarget === creep.room.storage.id
			) {
				// cycle detected
				const dummyCreep = {
					carry: {},
				...
				if (validAction && this.assign(creep, withdrawTarget)) {
					creep.data.nextAction = validAction.name;
					creep.data.nextTarget = nextTarget.id;
					return true;
				}
			} else {
				return this.assign(creep, withdrawTarget);
			}
		}
		return false;
	};
...
export default new WithdrawingAction();
