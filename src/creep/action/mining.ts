...
class MiningAction extends CreepAction {
	constructor() {
		super('mining');
		this.setDefault({
			newTarget: creep => {
				const addable = s => this.isValidTarget(s, creep) && this.isAddableTarget(s, creep);
				return (
					_(creep.room.sources)
						.sortBy(s => creep.pos.getRangeTo(s))
						.find(addable) || null
				);
			},
		});
	...
	newTarget = creep => {
		const target = creep.getStrategyHandler([this.name], 'newTarget', creep);
		if (target) {
			this.determineSpot(creep, target);
		}
		return target;
	};
	determineSpot = (creep, source) => {
		const invalid = [];
		...
		const containerSpot =
			source.container &&
			source.container.structureType === STRUCTURE_CONTAINER &&
			source.container.pos.isNearTo(source) &&
			!_.some(invalid, { x: source.container.pos.x, y: source.container.pos.y })
		...
		if (!creep.data.determinatedSpot) {
			Log.error('Unable to determine working location for miner in room ' + creep.pos.roomName);
		}
	};
	work = creep => {
				...
				if (repairTarget) {
					...
					if (LOG_TRACE)
						Log.trace('Action', {
							...
							creepName: creep.name,
							pos: creep.pos,
							repairTarget: repairTarget.id,
							progress: repairTarget.hits / repairTarget.hitsMax,
						});
				...
				if (buildTarget) {
					...
					if (LOG_TRACE)
						Log.trace('Action', {
							...
							creepName: creep.name,
							pos: creep.pos,
							buildTarget: buildTarget.id,
							progress: buildTarget.progress / buildTarget.progressTotal,
						});
...
export default new MiningAction();
