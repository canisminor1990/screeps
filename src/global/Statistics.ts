	...
	run = () => {
		...
		if (SEND_STATISTIC_REPORTS && this.storedStatisticsTime > 0) {
			message =
				...
				' (' +
				(Game.time - Memory.statistics.tick) +
				' loops)</h4>';

			if (Game.cpu.bucket) {
				const bucketDif = Game.cpu.bucket - Memory.statistics.bucket;
				...
				if (SEND_STATISTIC_REPORTS && this.storedStatisticsTime > 0) {
					...
					if (room.controller.my && room.memory.statistics) {
						...
						let totalDif = isUpgraded
							...
							: room.controller.progress - room.memory.statistics.controllerProgress;
						...
						message +=
							...
							step +
							'/loop )' +
							(isUpgraded ? ' <b><i>Upgraded!</i></b></li></ul>' : '</li></ul>');
						...
						if (room.storage && room.memory.statistics.store) {
							...
							for (let type in memoryStoreRecord) {
								// changed & depleted
								let dif = currentRecord[type]
						...
						message =
							'<ul><li><b>Room ' +
							room.name +
							'</b><br/><u>Controller</u><ul><li>Reservation: ' +
