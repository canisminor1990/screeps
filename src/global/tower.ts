...
mod.run = function(tower) {
	if (tower) {
		const p = Util.startProfiling(tower.room.name + ':tower:' + tower.id, {
			enabled: PROFILING.ROOMS,
		});
		...
		const closestHostile = tower.pos.findClosestByRange(tower.room.hostiles);
		if (closestHostile) {
			// Attack
