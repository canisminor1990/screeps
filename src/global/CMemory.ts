...
mod.saveSegment = (range, inputData) => {
	...
	for (let id = range.start; id >= range.end; id--) {
		if ((keys && keyNum < keys.length) || (encodedData && encodedData.length > 1)) {
			// more data to save
			if (!_.isUndefined(RawMemory.segments[id]) || numActive + CMemory.numSaved < 10) {
				let temp;
				let full = false;
				...
				if (_.isUndefined(RawMemory.segments[id])) CMemory.numSaved++;
			...
			} else if (numActive + CMemory.numSaved > 10) {
				// TODO: defer one tick?
			...
			if (DEBUG) Util.logSystem('CMemory.saveSegment', 'clearing unused segment ' + id);
			RawMemory.segments[id] = '';
			delete Memory.cacheValid[id];
		}
	}
...
mod.cleanup = () => {
	if (_.size(CMemory.toActivate) > 0) {
		RawMemory.setActiveSegments(Object.keys(CMemory.toActivate));
	}
	CMemory.toActivate = {};
	CMemory.numSaved = 0;
};
