...
const noMemoryWhere = function(e) {
	...
	return (
		setting === true ||
		(Memory.debugTrace.no.hasOwnProperty(e[0]) && (setting === e[1] || (!e[1] && setting === `${e[1]}`)))
	);
};
...
mod.DECAYABLES = [STRUCTURE_ROAD, STRUCTURE_CONTAINER, STRUCTURE_RAMPART];
mod.LAB_IDLE = 'idle';
mod.LAB_BOOST = 'boost';
mod.LAB_SEED = 'seed';
...
mod.logSystem = function(roomName, message) {
	...
	console.log(
		dye(CRAYON.system, `<a href="/a/#!/room/${Game.shard.name}/${roomName}">${text}</a> &gt; `) + message,
		Util.stack(),
	);
};
...
mod.processReports = function() {
	...
	if (
		...
		Memory.statistics.reports.length > 0
	) {
		let mails;
		// below max ?
		if (Memory.statistics.reports.length <= REPORTS_PER_LOOP) {
			...
			mails = chunks[0];
			Memory.statistics.reports = _(chunks)
				.tail()
				.concat();
		}
