...
mod.handleSpawningCompleted = creep => {
	...
	if (
		...
		creep.data.destiny.task != 'robbing'
	)
		return;
	...
	if (creep.pos.roomName == creep.data.homeRoom) {
		...
		if (carrySum > 0) {
			...
			if (deposit.length > 0) {
				...
				if (target.structureType == STRUCTURE_STORAGE && Creep.action.storing.assign(creep, target))
					return;
...
mod.storage = function(roomName, storageRoom) {
	...
	if (storageRoom) {
		...
		return `Task.${
			mod.name
		}: room ${roomName}, now sending haulers to ${storageRoom}, (was ${was})`;
