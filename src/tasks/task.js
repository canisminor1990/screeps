...
mod.register = function() {
	Task.tasks.forEach(task => {
		...
		// Flag Events
		if (task.execute && !Task.executeCache[task.name])
			Task.executeCache[task.name] = { execute: task.execute };
...
mod.validateAll = function(memory, flag, task, options = {}) {
	if (_.isUndefined(options.roomName))
		return logError('Task.validateAll', 'roomName undefined' + flag + options.subKey);
