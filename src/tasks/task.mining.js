...
mod.handleCreepDied = name => {
	...
	if (flag) {
		...
		Task.validateRunning(memory, flag, mod.name, {
...
mod.checkForRequiredCreeps = flag => {
	...
	const countExisting = type => {
		...
		Task.validateAll(memory, flag, mod.name, {
			roomName,
			subKey: type,
			queues: [priority],
			checkValid: true,
	...
	if (
		...
		for (let i = haulerCount; i < maxHaulers; i++) {
			...
			const storageRoomName = REMOTE_HAULER.REHOME
				? mod.strategies.hauler.homeRoomName(roomName)
				: spawnRoom.name;
...
mod.carry = function(roomName, partChange) {
	...
	return `Task.${mod.name}: hauler carry capacity for ${roomName} ${
...
mod.harvest = function(roomName, partChange) {
	...
	return `Task.${mod.name}: harvesting work capacity for ${roomName} ${
...
mod.carryPopulation = function(miningRoomName, homeRoomName) {
	...
	const neededWeight = Task.mining.strategies.hauler.maxWeight(
		miningRoomName,
		homeRoomName,
		undefined,
		false,
	...
	const totalWeight = Task.mining.strategies.hauler.maxWeight(
		miningRoomName,
		homeRoomName,
		undefined,
		true,
