...
mod.extend = function() {
	...
	Object.defineProperty(StructureStorage.prototype, 'charge', {
		// fraction indicating charge % relative to constants
		configurable: true,
		get: function() {
	...
	StructureStorage.prototype.getNeeds = function(resourceType) {
		...
		let order = null;
		if (storageData)
			order = storageData.orders.find(o => {
	...
	Object.defineProperty(StructureTerminal.prototype, 'charge', {
		// fraction indicating charge % relative to constants
		configurable: true,
		get: function() {
	...
	StructureTerminal.prototype.getNeeds = function(resourceType) {
		...
		let order = null;
		if (terminalData)
			order = terminalData.orders.find(o => {
				return o.type === resourceType;
			});
		if (!order) order = { orderAmount: 0, orderRemaining: 0, storeAmount: 0 };
		let loadTarget = Math.max(
