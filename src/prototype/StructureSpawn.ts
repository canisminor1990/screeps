	...
	run: {
		value() {
			...
			if (
				...
				if (
					!busy &&
					(room.spawnQueueMedium.length === 0 || room.spawnQueueMedium.length === spawnDelay.Medium)
				) {
	...
	createCreepByQueue: {
		value(queue, level) {
			...
			if (cost === 0) {
				Util.logSystem(
					this.pos.roomName,
					Util.dye(CRAYON.error, 'Zero parts body creep queued. Removed.'),
				);
				return false;
			}
			...
			for (
				let son = 1;
				completeName == null || Game.creeps[completeName] || Memory.population[completeName];
				son++
			) {
		...
		value(body, name, behaviour, destiny) {
			...
			if (name == newName || Util.translateErrorCode(newName) === undefined) {
				...
				if (CENSUS_ANNOUNCEMENTS)
					Util.logSystem(
						this.pos.roomName,
						Util.dye(CRAYON.birth, 'Good morning ' + newName + '!'),
					);
				return true;
			}
