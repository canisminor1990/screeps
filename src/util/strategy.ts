...
mod.decorateAgent = function(prototype, ...definitions) {
	...
	prototype.getStrategyHandler = function(ids, method, ...args) {
		...
		if (DEBUG && TRACE)
			Util.trace('Strategy', { agent: this.name, strategyKey, strategyName, method });
...
mod.buildStrategy = function(key, utils, definitions, custom) {
	...
	mod.appendstate(currentStrategy, undefined, [utils]);
	...
	for (let i = 0; i < definitions.length; i++) {
		...
		head = mod.appendstate(currentStrategy, head, state);
	}
	if (custom) {
		head = mod.appendstate(currentStrategy, head, [custom]);
	}
	if (head) {
...
mod.appendstate = function(currentStrategy, head, state) {
