 ...
 */
export class SpiralIterator {
	public y: number;
	public x: number;
	private _ring: number;
	private _dir: number; // dir of next step
	private _step: number;
	private _ringLimit: number;

	constructor(xy: Pos, ring: number, ringLimit: number) {
		// TODO borders
		this._dir = TOP;
		this.x = xy.x;
		if (ring === undefined) {
	...
	static dirTransform(origin: Pos, dir: number) {
		switch (dir) {
			case TOP_RIGHT:
				origin.x++;
				origin.y--;
