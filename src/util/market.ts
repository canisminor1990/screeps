export const marketUtils = {
	countPrices(orderType: string, mineral: string, roomName: string): number {
		...
		switch (orderType) {
			case ORDER_BUY:
				const allBuyOrders = prices(ORDER_BUY, mineral, MIN_MINERAL_SELL_AMOUNT, roomName);
				const minBuyOrder = _.min(allBuyOrders, 'ratio');
				const buyOrders = _.filter(allBuyOrders, order => {
			...
			case ORDER_SELL:
				const allSellOrders = prices(ORDER_SELL, mineral, TRADE_THRESHOLD, roomName);
				const maxSellOrder = _.max(allSellOrders, 'ratio');
				const sellOrders = _.filter(allSellOrders, order => {
