...
global.RawVisual = {
	...
	get data(){
		...
		return new Proxy(this._data,{ set: (t,k,v)=>(t[k]=v,this.dirty=true) })
	},
	commit(){
		if(this.dirty)
			this.save()
	...
	load(){
		...
		let data = this.raw
		try{
			if(data.slice(0,3) == 'lz:')
				data = lzstring.decompress(data.slice(3)) || '{}'
			if(data.slice(0,5) == 'lz64:')
				data = lzstring.decompressFromBase64(data.slice(5)) || '{}'
	...
	save(){
		...
		if(this.compression == 'lz')
			data = 'lz:'+lzstring.compress(data)
		if(this.compression == 'lz64')
			data = 'lz64:'+lzstring.compressToBase64(data)
		this.raw = data
		this.dirty = false
		let endCPU = Game.cpu.getUsed()
		let endTime = Date.now()
