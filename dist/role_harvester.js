...
roles.harvester.preMove = function (creep, directions) {
  ...
  var resources = _.filter(creep.room.getDroppedResources(), pickableResources);
  ...
  var reverse = creep.carry.energy === 0;

  if (creep.memory.routing.pathPos === 0) {
    for (var _resource in creep.carry) {
      if (_resource === RESOURCE_ENERGY) {
        continue;
      }
      creep.transfer(creep.room.storage, _resource);
    }
