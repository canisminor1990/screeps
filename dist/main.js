...
exports.default = function (creep) {
	...
	if (creep.room.name !== roomName) {
		if ((0, _Action.moveTo)(creep, new RoomPosition(24, 48, roomName))) return;
	} else {
		if ((0, _Action.attack)(creep, attackTarget)) return;
	}
};
...
exports.default = function (creep) {
	...
	if (!isFull) {
		...
		if (creep.room.storage.store.energy < _config2.default.terminal.storage && (0, _Action.transfer)(creep, creep.room.storage)) return;
...
exports.default = function (creep) {
	...
	if (isFull) {
		...
		if ((0, _Action.pickup)(creep, (0, _Action.findInRange)(creep, Memory.tasks[roonName].pickup, 4))) return;
		var container = Memory.flags[roonName].up;
		if (container) {
			container = Game.getObjectById(container.id);
			if (container.store.energy > 0 && (0, _Action.withdraw)(creep, container)) return;
		} else {
...
exports.default = function () {
	...
	try {
		...
		if (roomType == 'main') {
			switch (role) {
				case 'terminer':
					number = Game.rooms[roomName].storage.store.energy > _config2.default.terminal.storage + 50000 ? number : 0;
					break;
				case 'filler':
...
exports.default = function (room) {
	...
	_.forEach(room, function (roomGroup) {
		...
		// rcl
		var controller = roomMemory.structures.my.controller[0];
		...
		// storage
		var storage = roomMemory.structures.my.storage[0];
		try {
		...
		// terminals
		var terminal = roomMemory.structures.my.terminal[0];
		try {
		...
		// roles
		_.forEach(roomMemory.creeps.my, function () {
