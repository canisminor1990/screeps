...
Creep.prototype.moveRandom = function (onPath) {
  var start = Math.ceil(Math.random() * 8);
  var direction = 0;
...
Creep.prototype.moveRandomWithin = function (goal) {
...
Creep.prototype.moveCreep = function (position, direction) {
  ...
  if (creeps.length > 0 && creeps[0].memory) {
    ...
    if (this.memory.role === 'upgrader' && creeps[0].memory.role === 'storagefiller') {
      creeps[0].move(direction);
      return;
    }
    if (this.memory.role === 'upgrader' && creeps[0].memory.role === 'upgrader') {
      creeps[0].suicide();
      this.log('New killing');
      return;
    }
    if (this.memory.role === 'upgrader' && creeps[0].memory.role === 'sourcer') {
      this.log('config_creep_move suicide sourcer');
      creeps[0].suicide();
      return;
    }
    if (this.memory.role === 'upgrader' && creeps[0].memory.role === 'harvester') {
