...
roles.autoattackmelee.action = function (creep) {
  ...
  if (creep.room.controller.safeMode) {
    var _constructionSites = creep.room.find(FIND_CONSTRUCTION_SITES);
    creep.moveTo(_constructionSites[0]);
    return true;
  }
  ...
  if (creep.pos.getRangeTo(spawn.pos) <= 1) {
    ...
    var _structures = creep.pos.findInRange(FIND_STRUCTURES, 1);
    creep.cancelOrder('attack');
    var returnCode = creep.attack(_structures[0]);
  }
  return true;
};
