...
function getOppositeDirection(direction) {
  console.log('getOppositeDirection typeof: ' + (typeof direction === 'undefined' ? 'undefined' : (0, _typeof3.default)(direction)));
...
Creep.prototype.handle = function () {
  ...
  try {
    ...
    var message = 'Executing creep role failed: ' + this.room.name + ' ' + this.name + ' ' + this.id + ' ' + (0, _stringify2.default)(this.pos) + ' ' + err;
    if (err !== null) {
      message += '\n' + err.stack;
    }
...
Creep.prototype.isStuck = function () {
  return this.memory.last !== undefined && this.memory.last.pos3 !== undefined && this.pos.isEqualTo(this.memory.last.pos3.x, this.memory.last.pos3.y);
};
...
Creep.prototype.getEnergyFromStructure = function () {
  ...
  var area = this.room.lookForAtArea('structure', Math.max(1, this.pos.y - 1), Math.max(1, this.pos.x - 1), Math.min(48, this.pos.y + 1), Math.min(48, this.pos.x + 1));
  for (var y in area) {
    for (var x in area[y]) {
      ...
      for (var i in area[y][x]) {
        if (area[y][x][i].structureType === STRUCTURE_EXTENSION || area[y][x][i].structureType === STRUCTURE_SPAWN) {
...
Creep.prototype.buildRoad = function () {
  ...
  var creep = this;
  var buildableRoads = function buildableRoads(object) {
  ...
  var constructionSites = _.filter(this.room.getConstructionSites(), buildableRoads);
  ...
  if (constructionSites.length <= config.buildRoad.maxConstructionSitesRoom && (0, _keys2.default)(Game.constructionSites).length < config.buildRoad.maxConstructionSitesTotal && this.pos.inPath()) {
    var returnCode = this.pos.createConstructionSite(STRUCTURE_ROAD);
    if (returnCode === OK) {
      return true;
    }
...
Creep.prototype.moveForce = function (target, forward) {
  ...
  if (this.memory.lastPosition && this.pos.isEqualTo(new RoomPosition(lastPos.x, lastPos.y, lastPos.roomName))) {
  ...
  if (this.fatigue === 0) {
    if (forward) {
      ...
      var position = this.memory.path[this.room.name][+positionId];
...
Creep.prototype.killPrevious = function () {
  ...
  if (range === 1) {
    ...
    this.log('Kill previous' + ' ' + this.memory.role + ' ' + range + ' ' + (0, _stringify2.default)(previous));
    //    throw Error();
  }
};
...
Creep.prototype.spawnReplacement = function (maxOfRole) {
  if (this.memory.nextSpawn) {
    ...
    if (this.ticksToLive === this.memory.nextSpawn) {
      if (maxOfRole) {
        var creep = this;
        var creepOfRole = creep.room.find(FIND_MY_CREEPS, {
          filter: function filter(object) {
            if (object.memory.role === creep.memory.role) {
              return true;
            }
