...
roles.squadheal.preMove = function (creep, directions) {
  ...
  if (myCreeps.length > 0) {
    ...
    var range = creep.pos.getRangeTo(myCreeps[0]);
    if (range <= 1) {
  ...
  if (creep.memory.squad) {
    ...
    if (squad.action === 'move') {
      if (creep.room.name === squad.moveTarget) {
        var nextExits = creep.room.find(creep.memory.route[creep.memory.routePos].exit);
        var nextExit = nextExits[Math.floor(nextExits.length / 2)];
        var _range = creep.pos.getRangeTo(nextExit.x, nextExit.y);
        if (_range < 4) {
...
roles.squadheal.action = function (creep) {
  if (creep.hits < creep.hitsMax) {
    ...
    if (search.incomplete) {
      ...
      var _returnCode = creep.move(creep.pos.getDirectionTo(exit.x, exit.y));
      creep.log('rc: ' + _returnCode);
      return true;
    }
