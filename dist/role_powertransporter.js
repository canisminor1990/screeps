...
roles.powertransporter.action = function (creep) {
  ...
  var getResource = function getResource(creep) {
    if (creep.carry.power > 0) {
      creep.log(creep.memory.route[creep.memory.route.length - 2].room);
      var exitDirection = creep.room.findExitTo(creep.memory.route[creep.memory.route.length - 2].room);
      var nextExits = creep.room.find(exitDirection);
