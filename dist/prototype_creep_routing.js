...
Creep.prototype.getRoutePos = function (route) {
  ...
  var routePos = this.memory.routing.routePos || 0;
  // Detect room change
  if (!route[routePos] || this.room.name != route[routePos].room) {
    routePos = _.findIndex(route, function (i) {
      return i.room === _this.room.name;
    });

    // TODO if we can't find the room in the array
    if (routePos < 0) {
      this.log('newmove: No routepos found: ' + (0, _stringify2.default)(route));
    }
  }
...
Creep.prototype.getPathPos = function (route, routePos, path) {
  ...
  var pathPos = this.memory.routing.pathPos || 0;
  var pos = path[pathPos];

  if (!pos || !this.pos.isEqualTo(pos.x, pos.y)) {
    pathPos = _.findIndex(path, function (i) {
      return i.x === _this2.pos.x && i.y === _this2.pos.y;
    });
    if (pathPos === -1) {
      ...
      var posTarget = path[Math.floor(path.length / 2)];
      ...
      var returnCode = this.moveTo(posTarget.x, posTarget.y, {
        ignoreCreeps: true
      });
      if (returnCode != OK && returnCode != ERR_TIRED) {
        this.log('newmove: moveTo: ' + returnCode + ' ' + (0, _stringify2.default)(path[path.length / 2]) + ' ' + path.length / 2);
      }
      return -1;
    }
  }
...
Creep.prototype.getDirections = function (path, pathPos) {
  ...
  var pos = path[pathPos];
  if (!pos) {
    console.log('newmove: getDirections: pathPos: ' + pathPos + ' path: ' + (0, _stringify2.default)(path));
  }
  var currentPos = new RoomPosition(pos.x, pos.y, this.room.name);
  var forwardDirection = void 0;
  var backwardDirection = void 0;
  var direction = void 0;

  if (pathPos + 1 < path.length) {
    var nextPos = path[pathPos + 1];
    forwardDirection = currentPos.getDirectionTo(nextPos.x, nextPos.y);
  }

  if (pathPos - 1 >= 0) {
    var _nextPos = path[pathPos - 1];
    backwardDirection = currentPos.getDirectionTo(_nextPos.x, _nextPos.y);
  }
...
Creep.prototype.followPath = function (action) {
  ...
  var routePos = this.getRoutePos(route);
...
Creep.prototype.moveByPathMy = function (route, routePos, start, target, skipPreMove, action) {
  ...
  var path = this.room.getPath(route, routePos, start, target);
  if (!path) {
  ...
  var pathPos = this.getPathPos(route, routePos, path);
  if (pathPos < 0) {
    this.say('R:pos -1');
    ...
    if (path.length === 0) {
      this.log('config_creep_routing.followPath no pos: ' + (0, _stringify2.default)(path));
      return false;
    }
    ...
    var returnCode = this.moveTo(_.min(search.path, function (object) {
  ...
  var directions = this.getDirections(path, pathPos);
